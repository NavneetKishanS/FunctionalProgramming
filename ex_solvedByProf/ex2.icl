...
IsEvenDecimal x y = isEven (toInt ((x+y)*10.0))
...
ch2 :: Char -> Char
ch2 c
| x >= 65 && x <= 90 = toLower c // toChar (x+32)
| x >= 97 && x <= 122 = toUpper c
= c
where x = toInt c
...
isPerfectSquare x = y*y == x
where y = toInt (sqrt (toReal x))
...
transform x = toString (x/365) +++ " year " +++ toString (y/7) +++ " week " +++ toString (y rem 7) +++ " days"  
where y = x rem 365
