...
isoddnrs :: [Int] -> [Bool]
isoddnrs x = map (\x = isOdd(x) == True) x
...
sumsqr2 x = sum [a*a\\a<-[1..x]] // LAMBDA expr
...
f42_v2 x = map (\x = fa x) x
f42_v3 x = map fa x

//Start = f42_v3 [[1,2],[3,4,5,6],[7,8]] // [[1,4],[9,16,25,36],[49,64]]
...
avg :: [Real] -> Real
avg x = (foldr (+) 0.0 x)/toReal(length x) 
