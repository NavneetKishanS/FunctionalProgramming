...
= avg [studentGpa y\\y<-a.students]
//(sum [studentGpa y\\y<-a.students])/toReal(length a.students)
//We can use the build-in function avg to get the average of the list!

ExpandGrade :: [(String,[Int])]-> [(String, [Int])]
//ExpandGrade tupleList  = flatten[ [(name,[x]) \\ x <- list | x >= 85]  \\ (name,list) <- tupleList |(sum list) /(length list)> 75 ]
//ExpandGrade tupleList  = flatten[ [(name,[x]) \\ x <- list | x >= 85]  \\ (name,list) <- tupleList | avg list > 75 ]
ExpandGrade tupleList  = flatten[ [(name,[x]) \\ x <- list | x >= 85]  \\ (name,list) <- tupleList | length list >0 && avg list > 75 ]

//(sum list) /(length list) -> when the demoninator is zero,this will cause flashback
//avg will return "avg called with empty list"
//to avoid these errors,we can add one more condition like this: length list >0.
