...
//countOcc [] [] = []
//countOcc ls [x:xs] = [length (filter ( (==) x ) ls) : (countOcc ls xs)]

//Start = countOcc [1,1,1,1,1,2,3,5,6,6,8] [1,2,3,5,6,8]
...
gap _ [] = []
gap x [y:ys]
| y - x > 2 = [x+2 : gap (x+2) [y : ys]]
= [y : gap y ys] 

gap2 :: [Int] -> [Int]
gap2 [] = []
gap2 [x:xs] = [x : gap x xs] 
...
//Start = isNotPal 89
//Start = isPalindrome 78

getRidPal :: [[Int]] -> [[Int]]
getRidPal [[]] = [[]]
getRidPal ls = map (\x = filter (isNotPal) x) ls
...
removeNotPrime ls = filter (isNotPrime) ls
...
symSumGreater10 x = [ls ++ reverse ls \\ls <-x | sum ls > 10]
//[filter((<) 10)(map (\x = sum x) ls)]
...
elementInInterval [(l,r,ls)] = [[ls!!a \\ a<-[l..r]]]
//elementInInterval [(l,r,ls)] = [[ls!!a \\ a<-ls | a<=r && a>=l]]
