module midterm_solution_kristine
...
abundant :: Int -> Bool
abundant n = (sum[x \\ x<-[1..(n-1)] | n rem x == 0])>n
...
//Start = toList 145 // --> [1,4,5]

fact :: Int -> Int
fact x = prod[1..x]

strong_num :: Int -> Bool
strong_num n = (sum(map fact (toList n)))==n
...
index :: Int [Int] -> Int
index n list = hd[i \\ x <- list & i <- [0..] | x==n]


filtered :: Int [Int] -> [Int]
filtered k list = [k^(index x list) \\ x<-list | (x rem k)==0] 
...
retrieve list k = removeDup[(x,y) \\ x <- sortedList, y <- sortedList | abs(x-y)==k]
...
find_max_index [] = -1
find_max_index list = index (maxList list) list 
...
isPrime :: Int -> Bool
isPrime n = length[x \\ x<-[1..n] |(n rem x)==0]==2//1 and itself

find_prime_sum_pairs :: [Int] [Int] -> [(Int,Int)] 
find_prime_sum_pairs list1 list2 = [(x,y) \\ x <- list1, y <-list2 | isPrime (x+y)]
...
inc3 [x] = [x]
inc3 [x,y] = [x,y]
inc3 [x,y,z] = [x,y,(z+x+y)]
inc3 [x,y,z : xs] = (inc3 [x,y,z])++(inc3 xs)
...
mapFunList :: [Int] -> [[Int]]
mapFunList list = [[f a \\ f <- funList] \\ a <- list]
...
helper list = maxList[(((thd3 t)/(snd3 t))*100) \\ t<-list]

highestSval :: [(String,Int,Int)] -> [String]
highestSval list = [a \\ (a,b,c) <- list | ((c/b)*100)==(helper list)]//hd["peter"]
...
doubleUntilNeg list = (map (\x = x*2) (takeWhile ((<) 0) list)) ++ (filter (isEven) (dropWhile ((<) 0) list))

//(takeWhile ((<) 0) list) -> [1,2,3,-1,4,5,6] --> [1,2,3] --> [2,4,6]
//(dropWhile ((<) 0) list) -> [1,2,3,-1,4,5,6] --> [-1,4,5,6] --> [4,6]
...
indexA :: a [a] -> Int | Eq a // Eq a mean == operator should be defined on type a in CLEAN
indexA n list = hd[i \\ x <- list & i <- [0..] | x==n]


elementLocation :: [a] [a] -> [(a,Int)] | Eq a
elementLocation _ [] = []
elementLocation list1 [x:xs] 
|(isMember x list1) = [(x,(indexA x list1))] ++ (elementLocation list1 xs)
= [(x,(-1))] ++ (elementLocation list1 xs)
