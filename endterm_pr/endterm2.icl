...
sortedBookConcat :: {Book} {Book} -> {Book}
sortedBookConcat b1List b2List = {e \\ e<-(sort(removeDup([b \\ b<-:b1List] ++ [b \\ b<-:b2List])))}

//Start = sortedBookConcat lib1.books lib2.books
...
instance + Library
where
	(+) lib1 lib2 = {lib_name = lib1.lib_name +++ lib2.lib_name , books = (sortedBookConcat lib1.books lib2.books)}
...
instance == Library
where
	(==) l1 l2 = ([b \\b <-: l1.books]) == ([b \\ b<-: l2.books])
...
merge str1 str2 = [(charToStr a b)\\ a<-: str1 & b<-: str2] ++ [remainingChars str1 str2]

remainingChars :: String String -> String
remainingChars st1 st2
#b = [j \\ j<-: st2]
#a = [i \\ i<-:st1]
| length(a) == length(b) = ""
| length(a) > length(b) = {e \\ e<-(reverse(take ((length b)-1) (reverse a)))}
= {e \\ e<-(reverse(take ((length a)-1) (reverse b)))}

instance + {String}
where
	(+) st1 st2 = {stringListToString(merge i j)\\i<-:st1 & j<-:st2 }
...
height :: (Tree Int) -> Int
height Leaf = 0
height (Node _ le ri) = max(height le) (height ri) + 1

AVL_prop_check :: (Tree Int) -> Bool
AVL_prop_check Leaf = True
AVL_prop_check (Node _ l r) = abs (height l - height r) <= 1 && AVL_prop_check l && AVL_prop_check r

//AVL_prop_check (Node _ le ri)
//| ((height ri) - (height le)) <=1 = True//((height ri) - (height le))
//= False
...
treeToList Leaf = []
treeToList (Node x le ri) = [x] ++ treeToList le ++ treeToList ri

addLargerNums :: [Int] -> [Int]
addLargerNums [] = []
addLargerNums ls = [(e+ sum(filter((<)e)ls) )\\e<-ls]

addLargerNums2 :: Int [Int] -> Int
addLargerNums2 _ [] = 0
addLargerNums2 n ls = n+ sum(filter((<)n)ls) 

/*
transform :: (Tree Int) -> (Tree Int)
transForm Leaf = Leaf
transForm (Node x le ri) =  x + addLargerNums(treeToList le) + addLargerNums(treeToList ri)
*/
...
//Start = alterArray {(0,0,0),(1,-1,2), (-1,3,2), (-3,-1,-2)} // {(0,0,0),(1,1,4),(-1,-3,4)0,(-3,1,-4)}
...
//getOldest :: [EngagedPerson]->[EngagedPerson]
//getOldest ls = sort ls//.yearOfMarriage

getMinimum :: {(EngagedPerson, EngagedPerson)} -> Int
getMinimum arr = hd(sort[ a.yearOfMarriage \\ (a,b) <-: arr ])

lastName:: String -> String
lastName name= {e \\ e<-(tl(dropWhile((<>) ' ')[c \\ c<-:name]))}

getOldestMarriage :: {(EngagedPerson, EngagedPerson)} -> String
getOldestMarriage arr = hd[ lastName a.fullName \\ (a,b)<-:arr | a.yearOfMarriage == getMinimum arr]
