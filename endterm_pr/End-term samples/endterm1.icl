...
gradeAvg :: [Int] -> Int
gradeAvg gradesList = sum gradesList / length gradesList

getBestBScStudent :: [Student] -> String
getBestBScStudent [] = abort "Empty List"
getBestBScStudent studentList = hd [student.id \\ student <- studentList | gradeAvg student.grades == maxGrade]

where
	maxGrade = maxList [gradeAvg student.grades \\ student <- studentList | student.level == BSc]
...
split :: String -> [Char]
split st = fromString st

countChar :: [Char] Char -> Int
countChar ls c = length[x \\ x <- ls | x == c]

//count :: String -> {(Char, Int)}
//count st = {el \\ el <- (removeDup[(c,countChar (split st) c) \\ c <- split st | countChar (split st) c > 1])}
...
instance ~ [Int]
where
	 ~ ls = [newLs \\ newLs <- ls | newLs > 0]

instance - [Int]
where
	(-) l1 l2 = [n \\ n <- l1 | not (isMember n l2)]
...
findFakes accountArr = {new \\ new <- accList}

where
	accList = [acc \\ acc <-: accountArr]
...
diagSum :: [[Int]] -> Int
diagSum ls = sum [hd (drop i l) \\ l <- ls & i <- [0..]]

//Start = diagSum Mat1

//largestMat :: {[[Int]]} -> {[[Int]]}
largestMat :: {[[Int]]} -> [[[Int]]]
largestMat matArr = [mat \\ mat <-: matArr]
...
//Start = listToTree [1,10,2,20,3,4,5]
