...
bracketsStars _ 0 = ""
bracketsStars k n = toString ls +++ bracketsStars k (n-1)
...
convertChar c i
| i == 1 && toInt c <= 122 && toInt c >= 97 = toUpper c
| i > 1 && toInt c >= 65 && toInt c <= 90 = toLower c
= c

capitalize::String -> String
capitalize str = toString [convertChar c i \\ c <- charList & i <- [1..]]
...
same_major [] m = False
same_major [c:cs] m = c.major == m || same_major cs m
...
where
	(+) ls1 ls2
		| ls1 == sort ls1 && ls2 == sort ls2 = sort (ls1 ++ ls2)
		| ls1 == sort ls1 && ls2 <> sort ls2 = ls1
		| ls1 <> sort ls1 && ls2 == sort ls2 = ls2
...
//Start = treeToList tree1

instance == (Tree a) | == a
where
	(==) (Node x le ri) (Node y l r) = treeToList (Node x le ri) == treeToList (Node y l r)
...
avgList :: [Student] -> [Int]
avgList sList = reverse (sort [(sum g)/(length g) \\ g <- gradeList])

where 
	gradeList = [s.grades \\ s <- sList]

//Start = avgList [A, B, C]

//createStudTree :: [Student] -> (Tree Student)
//createStudTree [] = Leaf
//createStudTree sList = 
...
where
	...
	(==) Violet Violet = Amber
	(==) Teal Teal = Vermilion
	(==) Magenta Magenta = Chartreuse
	
	
find_complement :: Color -> Color
find_complement c = c == c
