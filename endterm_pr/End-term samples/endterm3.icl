...
isEvenLength :: {Int} Int -> Bool
isEvenLength arr n = length [num \\ num <-: arr | n == num] rem 2 <> 0

removeOcc :: {Int} -> {Int}
removeOcc arr = {n \\ n <-: arr | isEvenLength arr n}
...
generate :: Int -> {Int}
generate n = {num \\ num <- listOfN n 0}
...
//totalPrice :: [Shop] -> [Int]
//totalPrice sList = hd [hd [a \\ a <-: p] \\ p <- prodList]

//where
//	prodList = [s.products \\ s <- sList]

//Start = totalPrice [aldi,spar,lidl,abc]
...
swap v n1 n2
| isMember n1 v && isMember n2 v =  [n2 \\ e <- v | n1 == e] ++ [n1 \\ e <- v | n2 == e]
...
//getOldestMarriage couples = {c \\ c <-: couples}
