...
changeChar :: Char Int -> Char
changeChar c num = toChar (((toInt c - 65) + num) rem 26 + 65)

shiftCipher:: String Int -> String
shiftCipher str num = {changeChar x num \\ x <-: str}
...
//distance :: [Point] -> Real
//distance pLs = sqrt ((a.x - b.x)^2.0 + (a.y - b.y)^2.0)

//Start = distance car_location bus_location

//check :: Circle {Point} -> Bool
//check :: Circle {Point} -> [Point]
//check b pointArr = [p \\ p <-: pointArr]
...
instance == Book
where
	(==) bb1 bb2 = bb1.title == bb2.title && bb1.author == bb2.author && bb1.pyear == bb2.pyear && bb1.num_of_pages == bb2.num_of_pages
...
listArr :: {Book} {Book} -> [Book]
listArr books1 books2 = sort (removeDup ([b \\ b <-: books1] ++ [b \\ b <-: books2]))

//Start = listArr {b1, b2} {b1, b3}

instance + Library
where
	(+) lib1 lib2 = {lib_name = lib1.lib_name +++ lib2.lib_name , books = {b \\ b <- listArr lib1.books lib2.books}}
...
instance == Library
where
	(==) lib1 lib2 = [b \\ b <-: lib1.books] == [b \\ b <-: lib2.books]
...
stringToCharList str1 = [a \\ a <-: str1]

//Start = stringToCharList "hello" // ['h','e','l','l','o']

StringToString :: String -> String
StringToString str1 = {a \\ a <-: str1}

//Start :: String
//Start = StringToString "hello" // "hello"
...
//concatLs :: [Char] [Char] -> [Char]
//concatLs cl1 cl2 = [x \\ x <-: cl1 & x <-: cl2]

//Start = concatLs (charLs "hello") (charLs "wowow")
...
charToStr :: Char Char -> String
charToStr c1 c2 = fromChar c1 +++ fromChar c2

faux :: [String] -> String
faux [] = ""
faux [x:xs] = x +++ faux xs

merge :: String String -> [String]//[(Char,Char)]//String
merge str1 str2 = [(charToStr a b)\\ a<-: str1 & b<-: str2]

// +

//instance + {String}
//where
//	(+) st1 st2 = {faux(merge i j)\\i<-:st2 & j<-:st2 }
...
getTreeLength :: (Tree a) -> Int
getTreeLength Leaf = 0
getTreeLength (Node x le ri) = 1 + getTreeLength le + getTreeLength ri

//Start = getTreeLength tree1

AVL_prop_check :: (Tree Int) -> Bool
AVL_prop_check Leaf = True
AVL_prop_check (Node x le ri)
| abs (getTreeLength ri - getTreeLength le) > 1 = False
= True && AVL_prop_check le && AVL_prop_check ri
...
greaterNodesList :: (Tree Int) Int -> [Int]
greaterNodesList Leaf _ = []
greaterNodesList (Node x le ri) root
| x > root = [x] ++ (greaterNodesList le root) ++ (greaterNodesList ri root)
= (greaterNodesList le root) ++ (greaterNodesList ri root)

//Start = greaterNodesList BsTree1 5

transform :: (Tree Int) -> (Tree Int)
transform Leaf = Leaf
transform (Node x le ri)
| length (greaterNodesList (Node x le ri) x) == 0 = Node x (transform le) (transform ri)
= Node newX (transform le) (transform ri)

where
	newX = x + sum (greaterNodesList (Node x le ri) x)
...
//smallestTuple :: {(Int, Int, Int)} -> Int
//smallestTuple tuplesArr
//| minList [abs (fst3 x + snd3 x + thd3 x) \\ x <-: tuplesArr] == 


//Start = smallestTuple {(-1,3,2), (-3,-1,-2)} (-1,3,2)
