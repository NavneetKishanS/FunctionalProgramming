...
where
	toInt p = length [name \\ name <-: p.firstName] + p.age + p.height
...
continentsPrimeI ls = [c.contName \\ c <- ls | isMember 'i' [cc \\ cc <-: c.contName]]
...
shouldWePlay playersList goalKeeper = [p \\ p <-: playersList]
// | p.skillLevel >= goalKeeper.skillLevel] >= 3
