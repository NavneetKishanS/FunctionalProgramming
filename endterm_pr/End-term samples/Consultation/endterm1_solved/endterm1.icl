...
tree = Node 5 (Node 3 Leaf Leaf) (Node 2 Leaf (Node 4 Leaf Leaf))

level :: Int (Tree Int) Int -> Int
level n Leaf p = p
level n (Node x l r) p
|n == x = p
= max (level n l (p+1)) (level n r (p+1))
...
avgGrade :: [Int] -> Real
avgGrade list = toReal(sum list)/toReal(length list)

getBestBScStudent :: [Student] -> String
getBestBScStudent list 
| length maxStudents == 0 = abort "ERROR"
= (maxStudents!!0).id
where
	max = maxList [ avgGrade (student.grades) \\ student<-list  | student.level == BSc]
	maxStudents = [ student \\ student<-list | avgGrade student.grades == max]
...
cnt :: String Char -> Int
cnt s key = length[1 \\ w<-:s | w==key]

count :: String -> {(Char, Int)}
count arr = {a\\a<-list}
where 
	list = removeDup[(w,cnt arr w) \\ w<-:arr | cnt arr w > 1]
...
difference a b = [item \\ item<-a | not(isMember item b) ]

multiply :: [Int] [Int] -> [Int]
multiply l1 l2 = [ a*b \\ a<-l1 & b<-l2]

//Start = difference [1,2,3,4,5] [2,4]
...
instance - [Int]
where
	(-) a b = [num \\ num<-a | not(isMember num b)]
...
helper Leaf accum = accum
helper (Node x le ri) accum = (max (helper le (accum+1)) (helper ri (accum+1)) )

treeHeight :: (Tree Int) -> Int
treeHeight tree = helper tree 1
*/
...
btHelper :: BinaryTree Int -> Int
btHelper BTLeaf accum = accum
btHelper (BTNode le ri) accum = (max (btHelper le (accum+1)) (btHelper ri (accum+1)) )


getBTHeight :: BinaryTree -> Int
getBTHeight btree = btHelper btree 1
...
Insert (Pointer val linkedList) s = Pointer val (Insert linkedList s ) 
...
*/
helper :: (LinkedList String) -> [String]
helper Nil = []
helper (Pointer str next) =  helper next ++ [str] 

//Start = helper linkedlist1

putter :: (LinkedList String) [String] -> (LinkedList String)
putter Nil [] = Nil
putter (Pointer str next) [x:xs] = Pointer x (putter next xs)

Reverse :: (LinkedList String) -> (LinkedList String)
Reverse tree = putter tree (helper tree)
...
delete Nil str = Nil
delete (Pointer str next) key
| key == str = next
= Pointer str (delete next key)
...
concat Nil tree2 = tree2
concat (Pointer str next) tree2 = Pointer str (concat next tree2)
...
hasFake list key = (length[w \\ w<-list | w==key]) > 1

deque :: [BasicPersonAccount] -> [BasicPersonAccount]
deque [] = []
deque [x:xs]
| hasFake xs x= [{x & inQueue = False}] ++ deque xs
= [x] ++ deque xs

findFakes :: {BasicPersonAccount}-> {BasicPersonAccount}
findFakes arr = {w \\ w<-deqANDremoved}
where
	list = [w \\ w<-:arr]
	deqList = deque list
	deqANDremoved = removeDup[w \\ w<-deqList]
...
diagonalSum :: [[Int]] -> Int
diagonalSum list = sum[a!!n \\ a<-list & n<-[0..] ]

largestMat :: {[[Int]]} -> [[Int]]
largestMat arr = fst ((sortBy (\(x,y) (a,b) = y<b ) [(a,diagonalSum a) \\ a<-:arr])!!(size arr - 1))
...
toS :: (MyList Int) -> String
toS Empty = ""
toS (Elem a Empty) = toString a
toS (Elem a next) = (toString a) +++  "," +++ toS next
...
findWhich [] = 0
findWhich [(A a):xs] = (size a) + findWhich xs
findWhich [(B b):xs] = 1 + findWhich xs
