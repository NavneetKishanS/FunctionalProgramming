...
biggest Leaf2 = []
biggest (Node2 x left right)
| onlyRight (Node2 x left right) = [x] ++ biggest right
= biggest right ++ biggest left
...
getLastName name = toString (tl (dropWhile (\x = x <> ' ') [n \\ n <-: name]))

//Start = getLastName "Fsd A"
...
getOldestMarriage arr = hd[getLastName a.fullName \\ (a,b) <-: arr | getMinimum arr == a.yearOfMarriage]
