...
(--) t1 t2 = sqrt ((fst3 t1 - fst3 t2)^2.0 + (snd3 t1 - snd3 t2)^2.0 + (thd3 t1 - thd3 t2)^2.0)

//Start = (--) (1.0, 2.0, 3.0) (2.0, 2.0, 3.0)
...
removeVowels strArr2 = {{{c \\ c <-: s | not (isMember c vowelList)} \\ s <-: strArr} \\ strArr <-: strArr2}
...
FindLinesC s t = length [line \\ line <- Linelist |isMember s line.stops && line.transport == t] > 0
...
FindLinesR s = [(line.number, line.transport) \\ line <- Linelist | isMember s line.stops]
...
FindLinesB s1 s2 = [(line.transport, line.number) \\ line <- Linelist | isMember s1 line.stops && isMember s2 line.stops]
...
FindLinesT s1 s2 tr = [line.number \\ line <- Linelist | tr == line.transport && isMember s1 line.stops && isMember s2 line.stops]
...
/* -----------------------------------ABOUT TYPE SYNONYM---------------------------------------------------
with a type synonym one can define a new name for an existing type

In this example a new name is given to the type Int, namely Color. By defining
:: Color :== Int
Color has become a type synonym for the type Int.
...
add num set
|not (isMember num set) = sort [num:set]
= sort set
...
union set1 set2 = sort (set1 ++ [s \\ s <- set2 | not (isMember s set1)])

//Start = union [1,3,9] [2,4,6,8] // [1,2,3,4,6,8,9]
//Start = union [] [1..5] // [1,2,3,4,5]

intersection:: (Set a) (Set a) -> (Set a) | Eq a
intersection set1 set2 = [s \\ s <- set1 | isMember s set2]
...
treeToIntList :: (TreeT Int) -> [Int]
treeToIntList LeafT = []
treeToIntList (NodeT x le ri) = treeToIntList le ++ [x] ++ treeToIntList ri

//Start = treeToIntList tree1

goodNodes :: (TreeT Int) -> Int
goodNodes LeafT = 0
goodNodes (NodeT x le ri)
| x == (sum subTreesList) / (length subTreesList) = 1 + goodNodes le + goodNodes ri
= goodNodes le + goodNodes ri

where
	subTreesList = treeToIntList (NodeT x le ri) ++ treeToIntList (NodeT x le ri)
...
treeToIntList2 :: (Tree2 Int) -> [Int]
treeToIntList2 Leaf2 = []
treeToIntList2 (Node2 x le Leaf2) = treeToIntList2 le
treeToIntList2 (Node2 x Leaf2 ri) = [x] ++ treeToIntList2 ri
treeToIntList2 (Node2 x le ri) = treeToIntList2 le ++ treeToIntList2 ri

biggestNode :: (Tree2 Int) -> Int
biggestNode Leaf2 = 0
biggestNode tr = maxList [t \\ t <- (treeToIntList2 tr)]
